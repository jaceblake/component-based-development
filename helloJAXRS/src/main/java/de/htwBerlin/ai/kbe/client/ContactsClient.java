...
public class ContactsClient {
	...
	private static WebTarget baseTarget = client.target("http://localhost:8080/helloJAXRS/rest/contacts");
	...
	private static void listContacts() {
		...
		// Antwort gleich eine List von Contact-POJOs einlesen lassen
		List<Contact> contacts = baseTarget.request().get(new GenericType<List<Contact>>() {});
	...
	private static Contact getContact(long id) {
		...
		Contact contact = baseTarget.path(Long.toString(id)).request(MediaType.APPLICATION_XML).get(Contact.class);
	...
	private static void createContacts() {
		...
		for (int i = 1; i <= 5; i++) {
			...
			System.out.println("Posting new contact: " + entity.toString());
